import Head from 'next/head';

export default function RegisterNow() {
  return (
    <>
      <Head>
        <title>Register Now</title>
        <meta
          name="description"
          content="This page contains vital information about registering for SRVSO 2023"
        />
      </Head>
      <div className="mb-12">
        <h2 className="mb-4">2022-23 SRVSO Registration Instructions</h2>
        <p className="text-lg">
          This page contains vital information about registering for SRVSO 2023
        </p>
      </div>
      <h3 className="mt-20 text-lg">
        Please review this entire document to understand the registration
        process.
      </h3>
      <div className="flex flex-col w-full h-96">
        <iframe
          title="SRVSO Presentation"
          src="https://docs.google.com/document/d/1lAy0sDWO7MWHjSa-NkmJXOg50h70kCrVCOFhv4Y48kQ/edit?usp=sharing"
          allowFullScreen="true"
          mozallowfullscreen="true"
          webkitallowfullscreen="true"
          className="flex-grow"
        />
        <a
          href="https://docs.google.com/document/d/1lAy0sDWO7MWHjSa-NkmJXOg50h70kCrVCOFhv4Y48kQ/edit?usp=sharing"
          target="_blank"
          className="mt-2 text-sm text-center underline"
          rel="noopener noreferrer"
        >
          External link
        </a>
      </div>
      <h3 className="mt-20 text-lg">
        Note: Only Team Coaches Fill Out This Form
      </h3>
      <section className="flex flex-col w-full px-1 overflow-hidden bg-gray-200 rounded-md py-7 h-96">
        <iframe
          title="SRVSO Information Release Form"
          src="https://docs.google.com/forms/d/e/1FAIpQLScDIN_fDkTe66UmfC0hSGZ2PDiJxuGKC8g8sMl0k1kAt7ihrw/viewform?usp=sf_link"
          marginHeight="0"
          marginWidth="0"
          className="flex-grow"
        />
        <a
          href="https://docs.google.com/forms/d/e/1FAIpQLScDIN_fDkTe66UmfC0hSGZ2PDiJxuGKC8g8sMl0k1kAt7ihrw/viewform?usp=sf_link"
          target="_blank"
          className="mt-2 text-sm text-center underline"
          rel="noopener noreferrer"
        >
          External link
        </a>
      </section>
    </>
  );
}
